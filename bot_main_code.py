...
def at_answer(message):
    ...
    if text in ["start", "help", "english", "/start"]:
        ...
        bot.send_message(message.chat.id, const.menu,
                         reply_markup=bt.markupmenu)
        ...
        bot.send_photo(message.chat.id, const.dev_photo,
                       caption=consttr.beta, reply_markup=bt.markuplanguage)
        if text in ["turkish"]:
            languagecounter = 1
        # bot.send_sticker(message.chat.id,const.fire_thumbsUp)
        ...
        if character.inv() == 32:
            ...
            for item in character.inv():
                inventory = inventory + "\n-" + str(item)
        bot.send_message(message.chat.id, inventory,
                         reply_markup=bt.markupmenu)
    elif text in ["hunt", "ht"]:
        character.healcondition(0)
        monster = combat.pickmonster(character.lvl())
        result = combat.fight(monster.id, monster.hp, monster.power,
                              monster.xp, character.health(), character.power())
        if result == 255:
            character.hpconfig(100)
            bot.send_message(message.chat.id, const.healer,
                             reply_markup=bt.markupmenu)
        elif result == 0:
            character.hpconfig(0)
            bot.send_message(message.chat.id, const.lost,
                             reply_markup=bt.markupmenu)
        elif result[0] >= 1:
            character.repxp(result[0])
            character.dechp(result[1])
            win = "You defeat the " + str(monster.name) + " and earn " + str(
                result[0]) + " xp. And you lost " + str(result[1]) + " hp! You may rest!"
        ...
        bot.send_message(
            message.chat.id, "While resting, your hp will increase 1 per second. When your hp full you will be notified.", reply_markup=bt.markupmenu)
        ...
        if character.healing() == 1:
            bot.send_message(message.chat.id, "Your full! Current hp : " +
                             str(character.health()), reply_markup=bt.markupmenu)
        else:
            bot.send_message(message.chat.id, "Current hp : " +
                             str(character.health()), reply_markup=bt.markupmenu)
        ...
        bot.send_message(message.chat.id, profile_feedback,
                         reply_markup=bt.markupmenu)
    else:
        character.healcondition(0)
